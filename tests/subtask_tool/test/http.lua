...
function _M.send_request(self, params)
	...
	local is_proxy = params.is_proxy;
...
if not is_proxy then
	...
	if  ngx.ctx.trace_id  ~= nil then
		headers['trace_id'] = ngx.ctx.trace_id;
	end

    params.headers = headers
else
	if  ngx.ctx.trace_id  ~= nil then
		params.headers['trace_id'] = ngx.ctx.trace_id;
	end
	
	-- raw_header_ ["look_forward_ip"] 
	...
	local req ;
	if is_proxy then
		-- local headers_1 =  params.raw_header;
		-- req = _format_request(headers_1)
		params.headers = params.raw_header;
		req =  _format_request(params)
	else
		req =  _format_request(params)
		-- ngx.log(ngx.ERR, req);
		-- if params.method ~= "GET" then
		-- 	local cjson = require("cjson");
		-- 	ngx.log(ngx.ERR, "format =" .. req);
		-- 	ngx.log(ngx.ERR, "format2=" .. cjson.encode(ngx.req.get_headers()))
		-- 	ngx.log(ngx.ERR, "format3=" .. cjson.encode(ngx.req.raw_header()))
		-- 	ngx.log(ngx.ERR, "format4=" .. cjson.encode(ngx_req_get_headers()))
			
		-- 	ngx.log(ngx.ERR, "trace_id =" .. tostring(ngx.ctx.trace_id));
		-- end
		-- ngx.log(ngx.ERR, "format header\n\n");
	end
	-- local req = ngx.req.raw_header()
	-- ngx.log(ngx.ERR, "format" .. req);
...
function _M.proxy_request(self, chunksize,cparams, trace_id, look_forward_ip)
	local raw_header_ = ngx_req_get_headers();
	
	if trace_id ~= nil then
		raw_header_ ["trace_id"] = trace_id;
		
		if cparams ~= nil then
			cparams["trace_id"]  = trace_id;
		end
	end
	
	if look_forward_ip ~= nil then
		raw_header_ ["look_forward_ip"] = look_forward_ip;
		
		if cparams ~= nil then
			cparams["look_forward_ip"]  = look_forward_ip;
		end
	end
...
function _M.proxy_response(self, response, chunksize)
	...
	-- ngx.log(ngx.ERR, "status:" .. ngx.status);
    ...
    repeat
        ...
        if chunk then
			...
			-- ngx.log(ngx.ERR, tostring(chunk));
            if not res then
                ngx_log(ngx_ERR, err)
                break
            end
