...
function template.precompile(view, path, strip)
    ...
    if path then
		...
		if file == nil then
			ngx.log(ngx.ERR, "file " .. path .. "open failed");
		end
        file:write(chunk)
        file:close()
    end
        ...
        if t == LCUB then
            ...
            if e then
                ...
                if z then
					...
					c[j] = "___[#___+1]=template.escape(" .. trim(sub(view, p, e - 1)) .. ")" 
...
--]]
    ...
    local opt_O1 = function (v) 
		...
		local rets = concat(ret);
        rets = gsub (rets, "template.escape", "");
		--[[
		local file = open("debug.lua", "wb")
        file:write(rets)
        file:close()
		--]]
...
local function ngx_wasm_init()
	...
	if result ~= nil then
		...
		for k, v in ipairs(files) do
			...
			template[basename] = template.compile(v,basename);
			ngx.log(ngx.ERR, "[tml]" .. basename .. " add on");
		end
	end
end
	...
	if result ~= nil then
		...
		for k, v in ipairs(files) do
			...
			local compiled  = template.precompile(
					v, 
					"/home/admin/wasm/" .. basename ..".wasm");
			ngx.log(ngx.ERR, "[tml]" .. basename .. "compile done");
		end
	end
end
