...
#include "aes.h"
//#include "emscripten\1.35.0\system\include\emscripten\emscripten.h"
#include <stdio.h>
...
//	DVTUB_Print(Outdata,16);
//	return 0;
//}

void TA_HexToStr(uint8 *out, uint8 *in, int Len){ 
	char ddl = 0,ddh = 0; 
	int i = 0; 
...
void DVTUB_Print(uint8* data, int dataLen){ 
	int len = 0, offset = 0; 
	char g_printData[500];
	while(dataLen){ 
		if(100<dataLen){ 
			len = 100; 
			dataLen -= 100; 
		...
		TA_HexToStr(g_printData,data+offset, len);
		printf("%s", g_printData); 
		offset += len; 
	} 
} 
