...
local debugTrace = print;

local instance = loader.load(data)
local eax = instance.exports;
local ttt = 1;

--instance:link("env", "vm_random_double", function()
--  return  math.random();
--end)

instance:link("env", "vm_random_double", math.random);
...
while (step < 100000000) do
	local p = eax.award(0.01);
	if p == 0 then
		no = no +1;
	else
		yes= yes+1;
